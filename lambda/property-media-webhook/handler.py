...
WEBHOOK_PATH = "/internal/property-media/s3-uploaded"
...
def handler(event, context):
    for record in event.get("Records", []):
        ...
        if not key.startswith("media/property-staging/"):
            print(f"[SKIP] key={key} no pertenece al prefijo media/property-staging/")
            continue
